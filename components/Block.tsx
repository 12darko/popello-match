...
export const Block: React.FC<BlockProps> = ({ data, onClick, disabled, gridSize, wiggle, onWiggleEnd }) => {
  ...
  return (
    ...
    >
      {/* Jelly Layer (Behind block) */}
      {(data.jellyLevel || 0) > 0 && (
        <div className={`absolute inset-1 bg-fuchsia-500/40 rounded-[25%] blur-sm scale-110 z-[-1] animate-pulse`} />
      )}
      ...
      >
        ...
        {/* Chain Overlay */}
        {isChained && (
          <div className="absolute inset-0 bg-slate-900/50 rounded-[1.2rem] border-[3px] border-slate-400/60 flex items-center justify-center z-20">
            <div className="absolute w-full h-1.5 bg-slate-300 shadow-md rotate-45 border-y border-slate-400" />
            <div className="absolute w-full h-1.5 bg-slate-300 shadow-md -rotate-45 border-y border-slate-400" />
            <div className="bg-slate-800 p-1.5 rounded-full border-2 border-slate-400 shadow-lg z-10 relative">
              <Lock className="text-slate-200 w-5 h-5" />
              {(data.chainLevel || 0) > 1 && (
                <div className="absolute -top-1 -right-1 bg-red-500 text-white text-[8px] w-3 h-3 rounded-full flex items-center justify-center font-bold border border-white">
                  2
                </div>
              )}
            </div>
